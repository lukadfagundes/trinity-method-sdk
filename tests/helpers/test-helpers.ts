...
export async function createMockTrinityDeployment(
  ...
  // Create trinity directory structure (inside .claude/)
  await fs.ensureDir(path.join(targetDir, '.claude/trinity/knowledge-base'));
  await fs.ensureDir(path.join(targetDir, '.claude/trinity/templates'));

  // Create VERSION file
  await fs.writeFile(path.join(targetDir, '.claude/trinity/VERSION'), version);

  // Create user-managed knowledge base files
  await fs.writeFile(
    path.join(targetDir, '.claude/trinity/knowledge-base/ARCHITECTURE.md'),
    '# Architecture\n\nUser custom architecture content'
  );
  await fs.writeFile(
    path.join(targetDir, '.claude/trinity/knowledge-base/To-do.md'),
    '# To-Do\n\n- [ ] User task 1'
  );
  await fs.writeFile(
    path.join(targetDir, '.claude/trinity/knowledge-base/ISSUES.md'),
    '# Issues\n\nUser custom issues'
  );
  await fs.writeFile(
    path.join(targetDir, '.claude/trinity/knowledge-base/Technical-Debt.md'),
    '# Technical Debt\n\nUser custom debt tracking'
  );

  // Create SDK-managed knowledge base files
  await fs.writeFile(
    path.join(targetDir, '.claude/trinity/knowledge-base/Trinity.md'),
    '# Trinity Method\n\nOld version content'
  );
...
export async function readVersion(targetDir: string): Promise<string> {
  const versionPath = path.join(targetDir, '.claude/trinity/VERSION');
