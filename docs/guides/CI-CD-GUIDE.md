...
3. [BAS Quality Gates](#bas-quality-gates)
4. [Trinity Component Validation](#trinity-component-validation)
...
- Version tags (e.g., `v2.0.0`, `v2.0.1`)
- Manual workflow dispatch

### Deployment Jobs

1. **validate** - Pre-deployment validation (all tests, coverage, version check)
2. **build** - Build release artifacts
3. **publish** - Publish to npm registry
4. **github-release** - Create GitHub release
5. **notify** - Deployment success summary
...
1. **Validation** - All tests run, coverage checked, version verified
2. **Build** - Project built, artifacts verified
3. **Publish** - Published to npm registry (requires `NPM_TOKEN` secret)
4. **GitHub Release** - Release created with CHANGELOG notes
5. **Notify** - Deployment summary displayed
...
| `GITHUB_TOKEN` | Auto-generated by GitHub | Creating releases |
...
# Generate token (Automation type)
...
3. Ensure token has "Automation" type
