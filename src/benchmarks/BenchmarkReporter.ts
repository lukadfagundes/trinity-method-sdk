...
export class BenchmarkReporter {
  ...
  generateMarkdown(suite: BenchmarkSuite, outputPath: string): void {
    ...
    lines.push(`- **Failed:** ${suite.summary.failed} ${suite.summary.failed > 0 ? '❌' : ''}`);
  ...
  generateComparisonMarkdown(
    ...
    lines.push(`- **Regressions:** ${comparison.summary.regressions} ${comparison.summary.regressions > 0 ? '⚠️' : '✅'}`);
    lines.push(`- **Improvements:** ${comparison.summary.improvements} 📈`);
    lines.push(`- **Neutral:** ${comparison.summary.neutral} ➡️\n`);
    ...
    for (const [metric, diff] of Object.entries(comparison.differences)) {
      ...
      const status = diff.regression ? '⚠️ Regression' : diff.diff < 0 ? '📈 Improvement' : '➡️ Neutral';
